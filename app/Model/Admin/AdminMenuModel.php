    ...
    public static function mergeData($data)
    {
        if (!empty($data)) {
            foreach ($data as &$v) {
                ...
                //组合操作
                $v->handle       = '<a href="'.createUrl('Admin\Admin\AdminMenuController@getEdit',['id' => $v->id]).'" target="_blank" >修改</a>';
                //父级菜单
                if ($v['parent_id']) {
                    $v['parent_name'] = DB::table('admin_menu')->where(array('id' => $v['parent_id']))->pluck('menu_name');
