...
trait SetDateFormatFormHtmlBuildTraits
{
     ...
     * @author yangyifan <yangyifanphp@gmail.com>
     */
    public function buildFormDefaultValue($date_format = 'yyyy-MM-dd HH:mm:ss', $form_schema_name = null)
    {
        if ( is_null($form_schema_name)) {
            //设置最后一个表单字段时间格式
            $this->setFormDefaultValue(last($this->form_schema), $date_format);
        } else {
            //如果当前表单不存在,则初始化表单
            ...
            //设置指定的表单时间格式
            $this->setFormDefaultValue($this->form_schema[$form_schema_name], $date_format);
        }
        return $this;
    }
     ...
     * @author yangyifan <yangyifanphp@gmail.com>
     */
    private function setFormDefaultValue($form_chema_info, $date_format)
    {
        //获得当前表单字段的name名称
