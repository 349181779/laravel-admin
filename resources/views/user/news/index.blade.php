						...
						<?php if(!empty($all_category)):?>
							<?php foreach($all_category as $new_cat):?>
								<?php if($new_cat->id == (int)$_GET['id']):?>
									<li><a href="<?php echo action('User\NewsController@getIndex', ['id'=> $new_cat->id]);?>" class="select_a"><?php echo $new_cat->cat_name ;?></a></li>
								<?php else:?>
									<li><a href="<?php echo action('User\NewsController@getIndex', ['id'=> $new_cat->id]);?>"><?php echo $new_cat->cat_name ;?></a></li>
								<?php endif;?>
							<?php endforeach;?>
						<?php endif;?>
						<li><a href="<?php echo action('User\NewsController@getCategory');?>" >分类</a></li>
					</ul>
